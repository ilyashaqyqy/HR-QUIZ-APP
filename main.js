...
let timerInterval;
let sc = document.getElementById('finalScore');
...
function SuccessOrFail(finalscore) {
    if (finalscore >= 12) {
        window.location.replace(`Success.html?score=${finalscore}`);
    } else {
        window.location.replace(`failure.html?score=${finalscore}`);
    }
}
